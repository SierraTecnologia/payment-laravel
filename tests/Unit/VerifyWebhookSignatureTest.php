...
class VerifierMock
{
    ...
    public function __construct($webhookSecret)
    {
        ...
        $this->config->shouldReceive('get')->with('services.sierratecnologia.webhook.secret')->andReturn($webhookSecret);
        $this->config->shouldReceive('get')->with('services.sierratecnologia.webhook.tolerance')->andReturn(300);
    ...
    {
        $this->request->headers->set('SierraTecnologia-Signature', 't='.time().',v1='.$signature);
