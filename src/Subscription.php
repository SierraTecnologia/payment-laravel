...
use SierraTecnologia\Error\Card as SierraTecnologiaCard;
...
class Subscription extends Model
{
    ...
    {
        $class = Cashier::sierratecnologiaModel();
    ...
    public function incrementAndInvoice($count = 1)
    {
        ...
        $this->user->invoice(['subscription' => $this->sierratecnologia_id]);
    ...
    {
        $subscription = $this->asSierraTecnologiaSubscription();
    ...
    public function swap($plan, $options = [])
    {
        ...
        // that should be the expected behavior for most developers with SierraTecnologia.
        if ($this->quantity) {
        ...
        } catch (SierraTecnologiaCard $exception) {
            // When the payment for the plan swap fails, we continue to let the user swap to the
            // new plan. This is because SierraTecnologia may attempt to retry the payment later on. If
    ...
    public function resume()
    {
        ...
        // To resume the subscription we need to set the plan parameter on the SierraTecnologia
        // subscription object. This will force SierraTecnologia to resume this subscription
        // where we left off. Then, we'll set the proper trial ending timestamp.
        $subscription->plan = $this->sierratecnologia_plan;
    ...
    /**
     * Get the subscription as a SierraTecnologia subscription object.
     *
     * @return \SierraTecnologia\Subscription
     * @throws \LogicException
     */
    public function asSierraTecnologiaSubscription()
    {
        $subscriptions = $this->user->asSierraTecnologiaCustomer()->subscriptions;

        if (! $subscriptions) {
            throw new LogicException('The SierraTecnologia customer does not have any subscriptions.');
        }

        return $subscriptions->retrieve($this->sierratecnologia_id);
    }
}
